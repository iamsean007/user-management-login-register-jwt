    ...
    new Promise<string>((resolve, reject) => {
        bcrypt.genSalt(10, (err: Error | undefined, salt: string) => {
            ...
            bcrypt.hash(
                password,
                salt,
                (innerErr: Error | undefined, hash: string) => {
...
app.post(
    ...
    async (req: Request, res: Response): Promise<Response> => {
        try {
            ...
            return res.status(400).json({
                message: 'User Save Operation failed!',
            });
        } catch (err: any) {
            return res.status(500).json({
                message: 'An unexpected error occurred',
...
const attachUser = (req: any, res: Response, next: NextFunction): void => {
...
async function connect() {
    ...
    const tlsCertificateKeyFile = process.env
        .TLS_CERTIFICATE_KEY_FILE as string;
